...
var app = angular.module('cleaningApp', ['ngRoute','ngAnimate']);
...
app.controller('accountingCtrl',function ($scope) {

});
app.controller('homeCtrl', function ($scope ,$document, $window, $interval){
    $scope.showDropDown = false;
    $document.on('scroll', function() {
       $scope.deltaScroll = $window.scrollY;
        $scope.$apply(function() {
            $scope.pixelsScrolled = $window.scrollY;
        })
    });
    $scope.backgrounds = ['img/laptop.jpg','img/pen.jpg','img/257926-P4GHQ3-720.jpg','img/dell.jpg','img/table.jpg'];
    $scope.timer = -1;
    $interval(function () {
        $scope.timer ++;
        if($scope.timer< $scope.backgrounds.length){
            $scope.customBackground = 'background-image:url('+$scope.backgrounds[$scope.timer]+') !important';
        }else{
            $scope.timer= -1
        }
    },5000)
...
app.controller('mainCtrl', function ($scope, $window, $document ,$http ) {
    $document.on('scroll', function() {
        $scope.deltaScroll = $window.scrollY;
        $scope.$apply(function() {
            $scope.pixelsScrolled = $window.scrollY;
    ...
    $scope.sendForm = function () {
        $scope.postForm = {};
        $scope.postForm.userName = $scope.userName;
        $scope.postForm.userEmail = $scope.userEmail;
        $scope.postForm.userSubject = $scope.userSubject;
        $scope.postForm.userMessage = $scope.userMessage;

        $http.post('/mail', $scope.postForm).then(function success(response) {
...
app.controller('consultancyCtrl',function ($scope) {

});
app.controller('informationCtrl',function ($scope) {

});
app.controller('foundationsCtrl',function ($scope) {
